...
export default function EditToDoForm() {
  ...
  const getDataFromParam = () => {
    axios.get(`https://todo-backend-30x6.onrender.com/todos/id/${id}`).then((res) => { setData(res.data); setTitle(res.data.title); setDescription(res.data.description); setTask(res.data.task) })
  }
  useEffect(() => {
    getDataFromParam()
  }, [])
  ...
  const handleUpdatecard = () => {
    ...
    if (description.length  >= 25) {
      ...
      axios.patch(`https://todo-backend-30x6.onrender.com/todos/${id}`, data).then((res) => console.log(res.data)).then(() => navigate("/"))
    }
    else {
  ...
  const handleDeleteCard = () => {
    console.log("deke")
    axios.delete(`https://todo-backend-30x6.onrender.com/todos/${id}`).then((res) => alert("Card Deleted Successfully")).then(() => navigate("/"))
